...
const useVoteMatrix = (
  ...
  const generateRandomVoteMatrix = useCallback(() => {
    ...
    const groupBoundaries = [
      0,
      ...safeGroupSizes.map((size) => Math.floor((size / 100) * rows)),
      rows,
    ]
    ...
    for (let g = 0; g < consensusGroups; g++) {
      ...
      const adjustAgree =
        agreePercentage *
        (1 - agreePercentage / 100) *
        PROPORTIONAL_ADJUSTMENT_FACTOR
      const adjustDisagree =
        disagreePercentage *
        (1 - disagreePercentage / 100) *
        PROPORTIONAL_ADJUSTMENT_FACTOR
      const adjustPass =
        (100 - agreePercentage - disagreePercentage) *
        (1 - (100 - agreePercentage - disagreePercentage) / 100) *
        PROPORTIONAL_ADJUSTMENT_FACTOR
      ...
      let groupAgree = Math.min(
        Math.max(baseAgree + (Math.random() * 2 - 1) * adjustAgree, 0),
        100,
      )
      let groupDisagree = Math.min(
        Math.max(baseDisagree + (Math.random() * 2 - 1) * adjustDisagree, 0),
        100 - groupAgree,
      )
      let groupPass = Math.min(
        Math.max(basePass + (Math.random() * 2 - 1) * adjustPass, 0),
    ...
    while (reshuffledComments.size < commentsToReshuffle) {
      ...
      if (!reshuffledComments.has(commentIndex)) {
        ...
        const groupIndices = Array.from(
          { length: consensusGroups },
          (_, i) => i,
        )
        const agreeGroup = groupIndices.splice(
          Math.floor(Math.random() * groupIndices.length),
          1,
        )[0]
        const disagreeGroup = groupIndices.splice(
          Math.floor(Math.random() * groupIndices.length),
          1,
        )[0]
        ...
        const swapVotes = Math.floor(
          maxSwapVotes * reshuffleIntensity * Math.random(),
        )
        ...
        for (let i = 0; i < rows; i++) {
          if (
            i >= groupDistributions[agreeGroup].startIndex &&
            i < groupDistributions[agreeGroup].endIndex &&
            newMatrix[i][commentIndex] === -1 &&
            agreeSwapped < swapVotes
          ) {
            newMatrix[i][commentIndex] = 1
            agreeSwapped++
          } else if (
            i >= groupDistributions[disagreeGroup].startIndex &&
            i < groupDistributions[disagreeGroup].endIndex &&
            newMatrix[i][commentIndex] === 1 &&
            disagreeSwapped < swapVotes
          ) {
            newMatrix[i][commentIndex] = -1
            disagreeSwapped++
          }
  ...
  const handleVoteChange = useCallback(
    (participant, comment) => {
      setVoteMatrix((prevMatrix) => {
        ...
        newMatrix[participant][comment] =
          ((newMatrix[participant][comment] + 2) % 3) - 1 // Cycle through Agree -> Disagree -> Pass -> Agree
        return newMatrix
      })
    },
    [setVoteMatrix],
