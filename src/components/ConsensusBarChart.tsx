export const ConsensusBarChart = ({
  groups,
  comments,
  commentTexts,
  voteMatrix,
        ...
        {sortedComments.map((comment) => {
          ...
          if (voteMatrix && voteMatrix.length > 0 && !comment.numPasses) {
            voteMatrix.forEach((participantVotes) => {
              const vote =
                participantVotes[comment.commentId || comment.commentIndex]
              if (vote === 0) passes++
          ...
          const agreeVotePercent =
            totalVotes > 0 ? (agrees / totalVotes) * 100 : 0
          const disagreeVotePercent =
            totalVotes > 0 ? (disagrees / totalVotes) * 100 : 0
          const passVotePercent =
            totalVotes > 0 ? (passes / totalVotes) * 100 : 0
            ...
            <tr key={commentIndex}>
              ...
              <td className="consensus-cell">
                ...
                <div className="consensus-stats">
                  <div className="vote-breakdown">
                    <span className="agree-count" title={agrees.toString()}>
                      {Math.round(agreeVotePercent)}%
                    </span>
                    <span
                      className="disagree-count"
                      title={disagrees.toString()}
                    >
                      {Math.round(disagreeVotePercent)}%
                    </span>
                    <span className="pass-count" title={passes.toString()}>
                      {Math.round(passVotePercent)}%
                    </span>
              ...
              {groups &&
                ...
                groups.map((group, groupIndex) => {
                  ...
                  const groupTotalVotes =
                    groupAgrees + groupDisagrees + groupPasses

                  // Calculate percentages for this group
                  const groupAgreePercent =
                    (groupAgrees / totalParticipants) * 100
                  const groupDisagreePercent =
                    (groupDisagrees / totalParticipants) * 100
                  const groupPassPercent =
                    (groupPasses / totalParticipants) * 100
                  const groupNoVotePercent =
                    (groupNoVotes / totalParticipants) * 100

                  // Keep vote percentages out of those who voted in this group (for text display)
                  const groupAgreeVotePercent =
                    groupTotalVotes > 0
                      ? (groupAgrees / groupTotalVotes) * 100
                      : 0
                  const groupDisagreeVotePercent =
                    groupTotalVotes > 0
                      ? (groupDisagrees / groupTotalVotes) * 100
                      : 0
                  const groupPassVotePercent =
                    groupTotalVotes > 0
                      ? (groupPasses / groupTotalVotes) * 100
                      : 0
                  ...
                  return (
                    <td key={groupIndex} className="consensus-cell">
                      ...
                      <div className="consensus-stats">
                        <div className="vote-breakdown">
                          <span
                            className="agree-count"
                            title={groupAgrees.toString()}
                          >
                            {Math.round(groupAgreeVotePercent)}%
                          </span>
                          <span
                            className="disagree-count"
                            title={groupDisagrees.toString()}
                          >
                            {Math.round(groupDisagreeVotePercent)}%
                          </span>
                          <span
                            className="pass-count"
                            title={groupPasses.toString()}
                          >
                            {Math.round(groupPassVotePercent)}%
                          </span>
                          <span className="total-count">
                            ({groupAgrees + groupDisagrees + groupPasses})
                          </span>
