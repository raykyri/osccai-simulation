            ...
            <div className="matrix-scroll-container">
              <div className="matrix-content">
                {voteMatrix.map((row, i) => (
                  <div
                    key={i}
                    className={`matrix-row ${selectedGroup !== null && groups[selectedGroup]?.points.includes(i) ? "highlighted" : ""}`}
                  >
                    <div className="row-label">{i + 1}</div>
                    {row.map((vote, j) => (
                      <div
                        key={j}
                        className={`matrix-cell ${vote === 1 ? "agree" : vote === -1 ? "disagree" : vote === 0 ? "pass" : "none"} ${highlightedComment === j ? "highlighted" : ""}`}
