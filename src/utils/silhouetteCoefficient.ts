...
function calculateSilhouetteCoefficient(data, clusters) {
  ...
  const clusterPoints = Array.isArray(clusters[0])
    ? clusters
    : clusters.map((cluster) => cluster.points)

  debug("Clusters:", clusterPoints)

  const silhouetteScores = data.map((point, pointIndex) => {
    const clusterIndex = clusterPoints.findIndex((cluster) =>
      cluster.includes(pointIndex),
    )
    if (clusterIndex === -1) {
    ...
    const a = calculateAverageDistance(
      point,
...
function calculateAverageDistance(point, clusterPoints, data, pointIndex) {
  if (clusterPoints.length === 1) {
    return clusterPoints[0] === pointIndex
      ? 0
      : euclideanDistance(point, data[clusterPoints[0]])
  }
...
export function findOptimalClusters(points, startK = 2, endK = 9) {
  ...
  for (let seed = minSeed; seed <= maxSeed; seed++) {
    ...
    for (let k = startK; k <= maxK; k++) {
      ...
      try {
        ...
        const silhouetteCoeff = calculateSilhouetteCoefficient(
          points,
          formattedClusters,
        )
  ...
  const sums = allResults.map((res) =>
    res.map((val) => val[1]).reduce((sum, val) => sum + val, 0),
  )
